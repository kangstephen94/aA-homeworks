...
describe Dessert do
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new('brownie', 'alot', 'steve')}.to raise_error(ArgumentError)
    end
  end

  describe "#add_ingredient" do
    before(:each) do
      dessert.add_ingredient('kimchi')
    end

    it "adds an ingredient to the ingredients array" do
      expect(dessert.ingredients).to eq(['kimchi'])
    end
  ...
  describe "#eat" do
    it "subtracts an amount from the quantity" do
      expect(dessert.eat(40)).to eq(10)
    end

    it "raises an error if the amount is greater than the quantity" do
      expect {dessert.eat(60)}.to raise_error(StandardError)
    end
  end
